...
/* ---------- 3.  MEGA-MENU WITH WIPE & IMAGE ZOOM ---------- */
const NavItem = ({
  title,
  items,
  layoutId,
  ...
  return (
    ...
    >
      <button className="relative flex items-center gap-1 px-6 h-full text-black font-extrabold text-base hover:text-brand-primary transition-colors">
        {title}
        <FaChevronDown
            ...
            <div className="max-w-7xl mx-auto px-8 py-2">
              <div className="grid w-full gap-2 grid-cols-4">
                ...
                >
                  ...
                  <h5 className="font-extrabold text-brand-text mb-2 text-sm">Impact Story</h5>
                  <p className="text-gray-600 text-xs mb-3">See how we&apos;re changing lives</p>
                  <Link
                    href="/blog"
...
/* ---------- 4.  MOBILE DRAWER WITH REFUGEE-WAVE ---------- */
const MobileDrawer = ({ open, setOpen, navItems }: { open: boolean; setOpen: (v: boolean) => void; navItems: any }) => {
  const shouldReduceMotion = useReducedMotion();
  return (
    <AnimatePresence>
      {open && (
...
const ThemeToggle = () => {
  ...
  return (
    <button
      ...
      className="ml-4 text-white hover:text-brand-primary transition-colors"
  ...
  return (
    <>
      ...
      >
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-12 h-20">
            <Link href="/" className="flex items-center gap-4 group">
              ...
              >
                ...
                <p className="text-sm text-gray-600 font-medium">For Refugees And Immigrants</p>
              </motion.div>
            </Link>

            {/* Desktop menu */}
            <div className="hidden xl:flex items-center h-full gap-1">
              <NavItem title="About" items={navItems.about} layoutId="underline-about" />
              <NavItem title="Events" items={navItems.events} layoutId="underline-events" />
              <NavItem title="Programs" items={navItems.programs} layoutId="underline-programs" />
              <NavItem title="Get Involved" items={navItems.getInvolved} layoutId="underline-getInvolved" />
            </div>
        ...
        <MobileDrawer open={mobileOpen} setOpen={setMobileOpen} navItems={navItems} />
      </nav>
    </>
  );
};