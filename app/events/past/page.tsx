...
async function getPastEvents(): Promise<Event[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/events?category=Past`, {
    next: { revalidate: 60 },
  });
  if (!res.ok) return [];
  return res.json();
  ...
  return (
    <div className="min-h-screen bg-brand-background">
      <section className="relative pt-32 pb-16 px-6 md:px-12 text-center bg-gradient-to-b from-brand-primary/20 to-brand-background">
        <h1 className="text-5xl md:text-6xl font-black text-black mb-6">Past Events</h1>
      ...
      <section className="py-8 px-6 md:px-12 max-w-5xl mx-auto">
        <div className="flex flex-wrap justify-center gap-2">
          <Link href="/events" className="px-4 py-2 rounded-full text-sm font-extrabold bg-white text-brand-text hover:bg-brand-primary/20">
            Upcoming
          </Link>
          <Link href="/events/past" className="px-4 py-2 rounded-full text-sm font-extrabold bg-brand-primary text-black">
            Past
          </Link>
        </div>
      </section>
          ...
          {events.map((ev) => (
            <article key={ev.id} className="bg-white rounded-2xl shadow hover:shadow-xl transition-all overflow-hidden group">
              <Link href={`/events/${ev.slug}`}>
                ...
                <div className="p-5">
                  <span className="text-xs font-extrabold text-brand-primary uppercase tracking-wider">Past</span>
